...
sipa_diff_scores = df['sipa_post_score'] - df['sipa_pre_score']
...
fig, axes = plt.subplots(5, 2, sharey='row')
...
axes[2, 0].set_title('ATI')
sns.boxplot(ax=axes[2, 0], data=ati_df)

axes[3, 1].set_title('ESS')
sns.boxplot(ax=axes[3, 1], data=ess_df)

axes[4, 1].set_title('NASA-TLX')
sns.boxplot(ax=axes[4, 1], data=nasa_tlx_df)
